...
How to ship `retold-data-mapper` to npm and to GitHub Container Registry
(GHCR). Generated by `quack docker-init`; the structure matches the
shared template across all dockerized retold tools.

`retold-data-mapper` is a **long-running service**. Restart policy in compose / k8s should be `unless-stopped` (or equivalent). The Dockerfile should declare a HEALTHCHECK against the service's health endpoint.
...
- **Git remote configured** — `git remote get-url origin` should print
  `git@github.com:stevenvelozo/retold-data-mapper.git` (or the
  HTTPS equivalent).
...
BUILD_DOCKER=1 npm publish    (or: npm run publish:docker)
  ...
  → docker push ghcr.io/stevenvelozo/retold-data-mapper:<version>
```
...
1. **npm**: `npm view retold-data-mapper version`
2. **Workflow**: `https://github.com/stevenvelozo/retold-data-mapper/actions`
3. **Image**: `docker pull ghcr.io/stevenvelozo/retold-data-mapper:latest`
...
```bash
docker pull ghcr.io/stevenvelozo/retold-data-mapper:latest
docker run --rm ghcr.io/stevenvelozo/retold-data-mapper:latest
```
